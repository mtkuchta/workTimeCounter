...
class ProjectsMenu {
  constructor() {
    ...
    this.projectInput = document.querySelector(".projects-menu__input-project");
  }

  addProject(e, newProject) {
    e.preventDefault();
    const isExisted = this.projects.find(
      (project) => project.name.toUpperCase() === newProject.toUpperCase()
    )
      ? true
      : false;
    if (isExisted) {
    ...
    projects.forEach((project) => {
      ...
      const inputHTML = `
            <h2 class="project__title">${project.name}</h2>
             <span class="fas fa-caret-down expand ${
               project.isOpen === true ? "open" : ""
             }"></span>
      ...
      newProject.insertAdjacentHTML("beforeend", cleanHTML);
  ...
  addListenersToProject(project) {
    ...
    addTaskForm.addEventListener("submit", (e) =>
      this.addTask(e, addTaskForm.elements.newTask.value)
    );
  ...
  addTask(e, newTask) {
    e.preventDefault();
    const projectIndex = e.target.parentElement.dataset.index;
    const isExisted = this.projects[projectIndex].tasks.find(
      (task) => task.name.toUpperCase() === newTask.toUpperCase()
    )
      ? true
      : false;
  ...
  addListenersToTasks() {
    ...
    addTaskBtns.forEach((btn) =>
      btn.addEventListener("click", (e) => this.addTaskToActive(e))
    );
  }
      ...
      projects[index].tasks.forEach((task, taskIndex) => {
        ...
        const inputHTML = `
                ...
                <span class="fas fa-trash-alt delete-task"></span>
                    `;
        const cleanHTML = DOMPurify.sanitize(inputHTML);
        li.insertAdjacentHTML("beforeend", cleanHTML);
        ul.appendChild(li);
      });
    });
  ...
  deleteTask(e) {
    ...
    const activeProjectIndex =
      taskToDelete.parentElement.parentElement.dataset.index;
